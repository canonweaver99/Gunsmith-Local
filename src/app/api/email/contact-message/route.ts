...
export async function POST(request: NextRequest) {
  try {
    ...
    const { listingId, senderName, senderEmail, senderPhone, subject, message, contactMethod } = body
    ...
    const emailData = {
      ...
      message,
      contactMethod,
      listingUrl: `${process.env.NEXT_PUBLIC_SITE_URL || 'http://localhost:3000'}/listings/${listingId}`
    }

    await emailService.sendContactMessageNotification(emailData)
