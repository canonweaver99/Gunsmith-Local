...
import { Target, Wrench, Package, ChevronRight, ChevronLeft, Loader2, CheckCircle, Truck, Users } from 'lucide-react'
...
export default function WizardPage() {
  ...
  useEffect(() => {
    ...
    if (location) {
      setWizardData(prev => ({ ...prev, location }))
    }
  }, [searchParams])
        ...
        <section className="bg-gunsmith-accent/20 py-12 px-4">
          <div className="container mx-auto text-center">
            ...
            <p className="text-gunsmith-text-secondary">
              {wizardData.location && `Searching in ${wizardData.location} • `}
              Answer a few questions to get matched with specialists
            </p>
          </div>
        </section>
            ...
            <div className="card">
              ...
              {step === 2 && (
                <div className="space-y-6">
                  ...
                  {/* Parent categories as buttons; expand to show children */}
                  <div className="space-y-4">
                    {GUNSMITH_SPECIALTIES.map(group => (
                      <details key={group.key}>
                        <summary className="wizard-button text-left">
                        ...
                        <div className="grid grid-cols-2 gap-3 mt-3">
                          {group.items.map(item => (
                            <button
                              ...
                              className={`wizard-button ${
                                wizardData.services.includes(item) ? 'selected' : ''
                              } text-center`}
                            >
